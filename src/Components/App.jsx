...
import GerenciarListaWrapper from './components/GerenciarListaWrapper';
import JogadorLista from './components/JogadorLista';
import LoginOrganizador from './components/LoginOrganizador';
import CadastroOrganizador from './components/CadastroOrganizador';
...
export default function App() {
  ...
  const validarLista = () => {
    const confirmedPlayers = jogadores.filter(j => j.presente && !j.removido).map((j, i) => ({
      id: i + 1,
      name: j.nome
    }));
  ...
  return (
    <div className="gerenciar-page">
      <Routes>
        <Route path="/" element={<LoginOrganizador />} />
            ...
            {jogadores.map((jogador, index) => (
              ...
              >
                <span className="player-name">
                  {jogador.emoji ? `${jogador.nome} ${jogador.emoji}` : jogador.nome}
                </span>
                <div className="player-actions">
                  {!jogador.removido ? (
                    <>
                      <button className="remove-button" onClick={() => removerJogador(index)}>❌</button>
                      <button
